...
const { post_order : post_order_prod , post_order_with_random_pick_bot : post_order_random, get_user_apikeys_from_local } = require ( '../utils/exchanges/custom' ) // require ( '../utils/exchanges/alibae' )
// const {  post_orde r_bp } = require ( '../utils/exchanges/bp' )
...
const { findall , findone , updaterows } = require ( '../utils/db' )
const moment = require ( 'moment' )
const { MAP_WORKERTYPE } = require ( '../configs/common' )
const { enqueue_act_count_log } =require( '../utils/common' )
const { getstatsofarray } = require ( '../utils/stats')
let list_tradepair = [ 'BTC_USDT' ]
// let list_tradepair = [ 'BTC_USDT' , 'ETH_USDT' ]
const BINANCE_EP_SPOT_TICKER = `https://api.binance.com/api/v3/ticker/price?symbol=`
...
/* const get_user_ap ikeys_from_db = async ( )=>{
...
const conv_bin_ob_resp_to_proper_arrays = ( data )=>{ //  ( {data })=>{
  let a_bin_sellside_prices , a_bin_sellside_amounts , a_bin_buyside_prices , a_bin_buyside_amounts
  if ( data?.asks?.length ){
    a_bin_sellside_prices = data?.asks.map( el => +el[ 0 ])
    a_bin_sellside_amounts= data?.asks.map( el => +el[ 1 ])      
  } else { }
  if ( data?.bids?.length){
    a_bin_buyside_prices  = data?.bids.map( el => +el[ 0 ])
    a_bin_buyside_prices  = data?.bids.map( el => +el[ 1 ])
  } else {}
  return { a_bin_sellside_prices , a_bin_sellside_amounts , a_bin_buyside_prices , a_bin_buyside_amounts }
}
...
const main = async ( { MAX_STOP_SYMBOL_ITER_AT } )=>{
  ...
  arr_useremail_apikeys = await get_user_apikeys_from_local( ) // get_user_ apikeys_from_db ()
  let n_orders_placed = 0
//  let arr_useremail_a pikeys = await get_use r_apikeys_from_db ()
  for ( let idxsymbols = 0 ; idxsymbols < max_iter_symbols && idxsymbols < list_tradepair?.length ; idxsymbols se++ ){
    let tickersymbol_snake = list_tradepair [ idxsymbols ]
    let tickersymbol = tickersymbol_snake.replace ( /_/g, '' )
    let marketinfo = JSON.parse ( j_ticker_symbols [ tickersymbol_snake ] ) ; if ( marketinfo?.asset ){} else {marketinfo.asset = marketinfo.base ; marketinfo.market = marketinfo.quote ; }
    let ep = `${ BINANCE_EP_ORDERBOOK }` //    let ep = `${ BINANCE_EP_SPOT_TICKER }${ tickersymbol }`
    ...
    try {
      ...
      if ( resp?.data?.asks?.length ) {  // let { price : midprice } = resp?.data
        await rediscli.hset ( KEYNAMES?.REDIS?.REF_CANDLE_PRICE , tickersymbol_snake , midprice )
        let { midprice , buy_volume , sell_volume , buy_price_head , sell_price_head } = parse_orderbook ( { j_ob : resp?.data } )         //
        let stepsize = +midprice / REF_PRICE_DIVIDER_FOR_BIN_WIDTH
        console.log ( { midprice , buy_volume , sell_volume , stepsize , N_ORDER_BINS_A_SIDE , buy_price_head , sell_price_head  } ) // let stepsize = +midprice / N_ORDE R_BINS_A_SIDE
        let { a_bin_sellside_prices , a_bin_sellside_amounts , a_bin_buyside_prices , a_bin_buyside_amounts } = conv_bin_ob_resp_to_proper_arrays ( resp?.data )
        /**************** SELL SIDE ********/
        const TYPE = 'limitsell'
        for ( let idxbin = 0 ; idxbin < N_ORDER_BINS_A_SIDE ; idxbin ++ ) { 
          ...
          let jstats_of_amounts_array = getstatsofarray ( a_bin_sellside_amounts ) //      max , min , average , count ,     
          for ( let idxorder = 0 ; idxorder < n_orders ; idxorder ++ ) {
            let orderprice = get_random_float ( { max : bin_border_high   , min : bin_border_low }) //            let orderamount= get_random_float ( { max : MAX_ORDER_AMOUNT  , min : MIN_ORDER_AMOUNT })             // let orderprice = get_random_float ( { max : marketinfo?.LIMIT_PRICE_MAX , min: marketinfo?.LIMIT_PRICE_MIN  })
//            let jsta ts_of_array = getstatsofarray (  ) // resp?.data?.asks ) //     max , min , average , count ,     
            let orderamount = get_random_float ( { max : jstats_of_amounts_array?.max , min : jstats_of_amounts_array?.min } ) //            let orderamount = get_random_float ( { max : marketinfo?.LIMIT_AMOUNT_MAX , min : marketinfo?.LIMIT_AMOUNT_MIN })
//            let { useremail , apikey } = get_random_from_arr ( arr_useremail_apikeys )
            LOGGER ( { orderprice , orderamount } )
//            continue 
            let resp 
            if ( true ){ LOGGER ( `dbg point` )
              resp = await post_order_random ( { // })  post_order_bp ({
                asset : marketinfo?.asset , // currency ,
                market : marketinfo?.market , // pair ,
                type : TYPE ,
                side : 'sell',
                amount : orderamount,
                price : orderprice
...
// ma in ( { MAX_STOP_SYMBOL_ITER_AT : 10 } )
true && main ( { MAX_STOP_SYMBOL_ITER_AT : 5  } )
false && create_common_channel_socket ()
// ma in ( { MAX_STOP_SYMBOL_ITER_AT : 1  } )
module.exports = {
  main ,
...
const init_ping= async()=>{
  ...
  try { axios.post ( `${ SCHEDULER?.URL }:${ SCHEDULER?.PORT_HTTP }/workers/ping` , { name : MAP_WORKERTYPE?.MARKETMAKER }).then( console.log ) } 
