...
2-1)SET LIMIT : MARKET ORDER FREQ RATIO = 7 : 3 // THIS WAY, ORDERBOOK BEEFS UP AS TIME GOES
...
const { get_tickers, get_orderbook, post_order, post_order_with_random_pick_bot } = require ( '../utils/exchanges/custom' )
    ...
    if ( workertype ){ ;   } 
      ...
      switch ( workertype ){
        case 'DRIFTER' : 
          switch ( actiontype ){
            case 'START' :
              poisson_process_for_drift.start() // defi  ne_poisson_process ()
            break 
...
const fs=require('fs')  // const get_refprice = async ()=>{  }
const init_ref_prices = async () => {
...
const init_ping= async()=>{
  ...
  try { let resp  = await axios.post ( `${ SCHEDULER?.URL }:${ SCHEDULER?.PORT_HTTP }/workers/ping` , { name : MAP_WORKERTYPE?.DRIFTER } ) ;  console.log ( resp?.data ) }
...
const init = async ()=>{  
  ...
  false && await create_common_channel_socket ()
}
init()
setTimeout (async ()=>{ poisson_process_for_drift.start()} , 7000 )
